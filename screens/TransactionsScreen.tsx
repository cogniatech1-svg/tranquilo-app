...
import { maskMoney, DS, getPocketIcon, getPocketPalette } from '../lib/config'
...
export function TransactionsScreen({
  ...
  // ── Gasto por bolsillo (reemplaza "Top gastos" — mismo patrón que BudgetScreen) ──
  const spentByPocket = useMemo(() => {
    const acc: Record<string, number> = {}
    for (const e of expenses) acc[e.pocketId] = (acc[e.pocketId] ?? 0) + e.amount
    return acc
  }, [expenses])
